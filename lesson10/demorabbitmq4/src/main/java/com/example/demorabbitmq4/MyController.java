...
public class MyController {
    ...
    @RequestMapping("/send3/{message}")
    @ResponseBody
    public String queue2(@PathVariable("message") String message) {
        logger.info("Отправка сообщения в очередь codeforce1");
        String response = (String) rabbitTemplate.convertSendAndReceive("codeforce1",message);
        logger.info(String.format("Получение сообщения '%s'",response));
        return String.valueOf("Сообщение от листенера : " + response);
    }
}
